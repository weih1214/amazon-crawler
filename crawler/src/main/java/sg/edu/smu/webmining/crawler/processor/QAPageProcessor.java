...
public class QAPageProcessor implements PageProcessor {
    ...
    private String extractNextUrl(Document doc) {
        final String nextUrlString = doc.select("ul.a-pagination li.a-last a").attr("href");
        return nextUrlString;
    }

    private Boolean isCategoryPage(String pageURL) {
        final Matcher m = CATEGORY_URL_PATTERN.matcher(pageURL);
        return m.find();
    }

    private Boolean isQuestionPage(String pageURL) {
        final Matcher m = QUESTION_URL_PATTERN.matcher(pageURL);
        return m.find();
    }
    ...
    public void process(Page page) {
        ...
        if(isCategoryPage(pageURL)) {
            ...
            final Question question = new Question(questionElement, questionID);

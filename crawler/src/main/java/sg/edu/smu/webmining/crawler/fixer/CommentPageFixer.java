 ...
 */
public class CommentPageFixer implements PageProcessor {
  ...
  public CommentPageFixer(int cycleRetryTimes, int sleepTime, int retryTimes, String charset) {
  ...
  public static void main(String[] args) throws SQLException, IOException {
    ...
    final String[] seedpageList = new DBSeedpageManager().getFixerSeedpage(cf.getCommentFilePath());
      ...
      try (final MongoDBManager mongoManager = new MongoDBManager(cf.getMongoHostname(), cf.getMongoPort(), "CommentPage", "content")) {
        try (final MysqlFileManager mysqlFileStorage = new MysqlFileManager(cf.getMysqlHostname(), cf.getMysqlUsername(), cf.getMysqlPassword(), cf.getStorageDir())) {
          try (final ProxyNHttpClientDownloader downloader = new ProxyNHttpClientDownloader()) {
            ...
            Spider spider = Spider.create(new CommentPageFixer(cf.getCycleRetryTimes(), cf.getSleepTime(), cf.getRetryTimes(), cf.getCharset()))
